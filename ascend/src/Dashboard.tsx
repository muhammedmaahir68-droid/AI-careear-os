...
export default function Dashboard() {
  ...
  const tabs = [
    ...
    { id: "companies", label: "Target Companies", icon: <Building2 size={18} /> },
            ...
            <motion.div key="roadmap" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-2 space-y-4">
                <div className="p-5 rounded-2xl border border-blue-500/30 bg-gradient-to-r from-blue-600/10 to-indigo-600/10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                  <div>
                    <h3 className="text-base font-bold text-white">Full Branch Placement Curriculum Map</h3>
                    <p className="text-xs text-muted-foreground mt-1">Select any level or topic below to start your step-by-step learning journey and daily exam.</p>
                  </div>
                  <button
                    onClick={() => setActiveTab("daily")}
                    className="px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold whitespace-nowrap transition-colors flex items-center gap-1.5 shrink-0 cursor-pointer"
                  >
                    <Zap size={14} /> Start Level 1 Flow
                  </button>
                </div>
                <SyllabusModule

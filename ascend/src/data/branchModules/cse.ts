...
export const CSE_IT_MODULES: BranchModuleData[] = [
  {
    ...
    studyMaterial: {
      summary: `An array is a contiguous memory collection supporting O(1) index lookup.
The Two Sum problem asks to find two indices whose values sum to a target value.
Brute Force: O(n²) time using nested loops.
Optimized Approach: Hash Table storing complements (target - num) in a single pass → O(n) time and O(n) space.`,
      keyPoints: [
        "Arrays provide O(1) random access by memory offset.",
        "Hash Map lookup is O(1) average time complexity.",
        "Complement formula: required_value = target - current_element.",
        "Space-Time trade-off: Trade O(n) memory to drop time from O(n²) to O(n)."
      ],

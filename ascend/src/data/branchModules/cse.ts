...
type CSETopic = {
  title: string; level: string; concept: string; keyPoints: string[];
  example: string; complexity: string; problem: string; input: string; output: string;
  starter: string; company: string;
};

const cseModule = (topic: CSETopic): BranchModuleData => ({
  moduleTitle: topic.title,
  level: topic.level,
  branch: ["cse", "it"],
  videos: makeVideoLinks(topic.title),
  studyMaterial: {
    summary: topic.concept,
    deepDiveTextbook: `Interview chapter: ${topic.title}. Start from the invariant, state the representation, then justify each operation with its cost. Production solutions must define failure cases, test boundaries, and choose the simplest data structure that preserves the invariant.`,
    keyPoints: topic.keyPoints,
    example: topic.example,
    complexity: topic.complexity,
    flowchartSteps: ["Identify inputs and constraints", "Choose a representation", "Maintain the core invariant", "Test edge cases", "Measure time and space"],
  },
  aiExplain: {
    steps: ["Restate the goal and constraints.", "Identify the invariant that must remain true.", "Select the data structure that makes the invariant cheap.", "Trace a normal case and an edge case.", "Explain the resulting time and space cost."],
    analogy: `Think of ${topic.title} as a carefully organized workflow: each rule prevents work from being repeated or lost.`,
  },
  debug: [{
    title: `Fix an edge case in ${topic.title}`,
    buggy: "def solve(items):\n    return items[0]  # fails for an empty input",
    fixed: "def solve(items):\n    if not items:\n        return None\n    return items[0]",
    hint: "Validate empty inputs before indexing and make the function's return contract explicit.",
  }],
  quiz: [
    { q: `What should you establish before implementing ${topic.title}?`, options: ["A correctness invariant", "A random output", "Only UI colors", "A database password"], answer: 0 },
    { q: "Which test is essential for collection algorithms?", options: ["Empty input", "Only a large monitor", "A new font", "A browser refresh"], answer: 0 },
    { q: "Why state complexity in an interview?", options: ["It exposes scalability trade-offs", "It removes testing", "It replaces code", "It changes syntax"], answer: 0 },
    { q: "What makes a useful invariant?", options: ["It remains true after every step", "It is a variable name", "It is always random", "It avoids constraints"], answer: 0 },
    { q: "What should a robust solution document?", options: ["Boundary behavior", "Only happy paths", "Only imports", "Only comments"], answer: 0 },
  ],
  mnc: [{ company: topic.company, year: "2024", question: `Explain your approach to ${topic.title} and its trade-offs.`, answer: `Define the invariant, walk through one example, and justify ${topic.complexity}.` }],
  mock: [{ type: "Technical", question: `How would you test ${topic.title}?`, tip: "Cover empty, singleton, duplicate, extreme, and representative inputs; then verify the claimed complexity." }],
  coding: { problem: topic.problem, desc: topic.concept, input: topic.input, output: topic.output, starter: topic.starter },
});

const CSE_EXPANDED_MODULES: BranchModuleData[] = [
  { title: "Stacks, Queues & Monotonic Patterns", level: "Level 2 - Data Structures", concept: "Stacks model last-in-first-out work; queues model first-in-first-out work. A monotonic stack retains only candidates that can influence the next greater or smaller answer.", keyPoints: ["Use stacks for matching delimiters and undo.", "Use queues for BFS and scheduling.", "Monotonic stacks make each element enter and leave once."], example: "def next_greater(a):\n    ans=[-1]*len(a); st=[]\n    for i,x in enumerate(a):\n        while st and a[st[-1]] < x: ans[st.pop()] = x\n        st.append(i)\n    return ans", complexity: "Time: O(n) | Space: O(n)", problem: "Next Greater Element", input: "[2, 1, 2, 4, 3]", output: "[4, 2, 4, -1, -1]", starter: "def next_greater(nums):\n    # maintain indices in a decreasing stack\n    pass", company: "Amazon" },
  { title: "Heaps, Priority Queues & Top-K", level: "Level 3 - Data Structures", concept: "A binary heap keeps the minimum or maximum item accessible while supporting incremental updates. It is the standard tool for Top-K, streaming medians, and Dijkstra's frontier.", keyPoints: ["Heap root is always extremal.", "Keep a size-k min heap for largest k values.", "Lazy deletion may simplify priority updates."], example: "import heapq\ndef top_k(a,k):\n    return heapq.nlargest(k,a)", complexity: "Time: O(n log k) | Space: O(k)", problem: "Kth Largest Element", input: "nums=[3,2,1,5,6,4], k=2", output: "5", starter: "def kth_largest(nums, k):\n    pass", company: "Microsoft" },
  { title: "Tries & Prefix Search", level: "Level 3 - Data Structures", concept: "A trie stores strings by shared prefixes, allowing prefix lookup without scanning every word. Nodes represent prefixes and terminal markers represent complete words.", keyPoints: ["Search cost depends on word length.", "Terminal flags distinguish a word from a prefix.", "Tries support autocomplete and dictionary matching."], example: "class Trie:\n    def __init__(self): self.next={}, False", complexity: "Insert/Search: O(L) | Space: O(total characters)", problem: "Implement Prefix Search", input: "words=['app','apple'], prefix='app'", output: "['app','apple']", starter: "def starts_with(words, prefix):\n    pass", company: "Google" },
  { title: "Union-Find & Connected Components", level: "Level 4 - Graph Algorithms", concept: "Disjoint Set Union maintains non-overlapping groups. Path compression and union by rank make repeated connectivity queries effectively constant time.", keyPoints: ["find returns a component representative.", "union merges representatives.", "Use DSU for cycle detection in undirected graphs."], example: "def find(p,x):\n    if p[x]!=x: p[x]=find(p,p[x])\n    return p[x]", complexity: "Amortized: O(alpha(n)) per operation", problem: "Number of Provinces", input: "edges=[[0,1],[1,2],[3,4]]", output: "2", starter: "def count_components(n, edges):\n    pass", company: "Meta" },
  { title: "Dynamic Programming: 1D & Knapsack", level: "Level 4 - Algorithms", concept: "Dynamic programming stores answers to overlapping subproblems. Define state, transition, base cases, and an order in which dependencies are already computed.", keyPoints: ["State must capture future-relevant information.", "Memoization and tabulation solve the same recurrence.", "Space can often be compressed when only the previous row is needed."], example: "def climb(n):\n    a,b=1,1\n    for _ in range(n): a,b=b,a+b\n    return a", complexity: "Typical: O(states × transitions)", problem: "0/1 Knapsack", input: "weights=[1,3,4], values=[15,20,30], capacity=4", output: "35", starter: "def knapsack(weights, values, capacity):\n    pass", company: "Walmart" },
  { title: "Dynamic Programming: Subsequences & Intervals", level: "Level 5 - Algorithms", concept: "Subsequence and interval DP solve structured choices over prefixes or ranges. The key is a state that identifies what has already been decided.", keyPoints: ["LCS uses two prefix indices.", "Interval DP grows ranges by length.", "Reconstruct an answer by retaining choices."], example: "def lcs(a,b):\n    dp=[[0]*(len(b)+1) for _ in range(len(a)+1)]\n    return dp", complexity: "LCS: O(mn) time | O(mn) space", problem: "Longest Common Subsequence", input: "a='abcde', b='ace'", output: "3", starter: "def lcs_length(a, b):\n    pass", company: "Adobe" },
  { title: "Greedy Algorithms & Proofs", level: "Level 4 - Algorithms", concept: "Greedy algorithms make a locally optimal choice only when an exchange argument or stays-ahead proof establishes global correctness.", keyPoints: ["Sort by the correct decision key.", "Prove the greedy choice is safe.", "Counterexamples expose invalid greedy rules."], example: "def select(intervals):\n    end=-float('inf'); count=0\n    for s,e in sorted(intervals,key=lambda x:x[1]):\n        if s>=end: count+=1; end=e\n    return count", complexity: "Usually O(n log n) due to sorting", problem: "Activity Selection", input: "[[1,3],[2,5],[4,7],[6,9]]", output: "2", starter: "def max_non_overlapping(intervals):\n    pass", company: "Goldman Sachs" },
  { title: "Backtracking & Constraint Search", level: "Level 4 - Algorithms", concept: "Backtracking explores a decision tree, immediately undoing choices that violate constraints. Pruning is what turns brute force into a practical search.", keyPoints: ["Choose, recurse, unchoose.", "Prune impossible partial states early.", "Use sets/bitmasks for fast constraint checks."], example: "def subsets(a):\n    out=[]\n    def go(i,path):\n        if i==len(a): out.append(path[:]); return\n        go(i+1,path); path.append(a[i]); go(i+1,path); path.pop()\n    go(0,[]); return out", complexity: "Worst case exponential", problem: "Generate Valid Parentheses", input: "n=3", output: "['((()))','(()())',...]", starter: "def generate_parentheses(n):\n    pass", company: "Uber" },
  { title: "Database Design & Normalization", level: "Level 3 - Core CS", concept: "Relational design uses keys and functional dependencies to prevent update anomalies. Normal forms balance data integrity with practical query needs.", keyPoints: ["Primary keys uniquely identify rows.", "3NF removes transitive dependency.", "Indexes accelerate reads but add write cost."], example: "CREATE TABLE enrollment (student_id INT, course_id INT, PRIMARY KEY(student_id, course_id));", complexity: "Indexed lookup commonly O(log n)", problem: "Design Course Enrollment Schema", input: "students, courses, enrollments", output: "normalized tables with keys", starter: "-- Write CREATE TABLE statements with primary and foreign keys", company: "Oracle" },
  { title: "SQL Joins, Windows & Query Tuning", level: "Level 4 - Core CS", concept: "SQL joins combine related rows; window functions calculate over partitions without collapsing them. Query plans reveal whether an index or rewrite is needed.", keyPoints: ["INNER JOIN keeps matches; LEFT JOIN preserves left rows.", "ROW_NUMBER ranks within a partition.", "Avoid SELECT * on large production paths."], example: "SELECT user_id, ROW_NUMBER() OVER(PARTITION BY team ORDER BY score DESC) AS r FROM scores;", complexity: "Depends on indexes, join strategy, and cardinality", problem: "Second Highest Salary", input: "Employee(id, salary)", output: "second distinct salary", starter: "SELECT /* write query */;", company: "TCS" },
  { title: "Operating Systems: Processes, Threads & Synchronization", level: "Level 3 - Core CS", concept: "Processes isolate resources; threads share an address space. Synchronization protects shared state while avoiding deadlock, starvation, and races.", keyPoints: ["A mutex provides mutual exclusion.", "Semaphores coordinate resource counts.", "Deadlock needs mutual exclusion, hold-and-wait, no preemption, and circular wait."], example: "# Critical section\nlock.acquire()\ntry: counter += 1\nfinally: lock.release()", complexity: "Lock acquisition depends on contention", problem: "Bounded Buffer", input: "producer and consumer threads", output: "no race or overflow", starter: "# Use a mutex and two semaphores\n", company: "Intel" },
  { title: "Computer Networks: TCP, HTTP & DNS", level: "Level 3 - Core CS", concept: "Networks layer addressing, reliable transport, and application protocols. TCP provides ordered reliable byte streams; HTTP defines request-response semantics; DNS resolves names.", keyPoints: ["TCP handshake establishes sequence numbers.", "HTTPS is HTTP over TLS.", "Caching and idempotency shape reliable APIs."], example: "GET /api/profile HTTP/1.1\nHost: example.com", complexity: "Latency includes DNS, handshake, and transfer", problem: "Design a Retry-Safe API Call", input: "POST payment request", output: "idempotency-key protected request", starter: "# Describe headers and retry logic", company: "Cisco" },
  { title: "System Design: Caching, Load Balancing & Queues", level: "Level 5 - System Design", concept: "Scalable services distribute load, cache hot reads, and use queues to decouple slow work. Every component needs a consistency and failure strategy.", keyPoints: ["Cache-aside reads populate on miss.", "Load balancers need health checks.", "Queues smooth bursts and permit retries."], example: "request -> load balancer -> service -> cache/database -> queue worker", complexity: "Design around throughput, p95 latency, and availability", problem: "Design URL Shortener", input: "create and redirect URLs", output: "API, storage, cache, and ID strategy", starter: "# Define API, data model, and scaling path", company: "Netflix" },
  { title: "Software Testing & Clean Code", level: "Level 2 - Engineering Practice", concept: "Reliable software is designed for testability. Unit tests isolate behavior, integration tests validate boundaries, and clean interfaces reduce the blast radius of change.", keyPoints: ["Arrange-Act-Assert keeps tests readable.", "Mock external boundaries, not implementation details.", "Small cohesive functions are easier to verify."], example: "def test_total_empty_cart():\n    assert total([]) == 0", complexity: "Test suites trade execution time for confidence", problem: "Test a Cart Total Function", input: "items with prices and quantities", output: "unit tests for normal and edge cases", starter: "def test_total():\n    pass", company: "Atlassian" },
  { title: "Git, CI/CD & Release Engineering", level: "Level 2 - Engineering Practice", concept: "Version control and automated pipelines make changes reviewable, reproducible, and safe to release. Small commits and gated checks reduce deployment risk.", keyPoints: ["Rebase or merge deliberately; resolve conflicts with tests.", "CI should lint, test, build, and scan.", "Feature flags separate deployment from release."], example: "git checkout -b feature/search\ngit commit -m 'Add prefix search'", complexity: "Pipeline duration is constrained by its slowest stage", problem: "Build a Safe Deployment Pipeline", input: "web service repository", output: "test, build, staging, rollback plan", starter: "# List CI stages and rollback trigger", company: "GitHub" },
  { title: "Security Fundamentals: Auth, OWASP & Cryptography", level: "Level 4 - Engineering Practice", concept: "Application security protects identity, data, and execution boundaries. Use established primitives, least privilege, and server-side validation rather than inventing crypto.", keyPoints: ["Hash passwords with salted adaptive functions.", "Authorize every sensitive action server-side.", "Parameterized queries prevent SQL injection."], example: "cursor.execute('SELECT * FROM users WHERE email = %s', (email,))", complexity: "Security controls add bounded overhead for risk reduction", problem: "Secure Login Flow", input: "email and password", output: "hashed password, session, authorization checks", starter: "# Outline validation, hashing, and session steps", company: "PayPal" },
].map(cseModule);
